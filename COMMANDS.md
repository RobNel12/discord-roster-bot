...
Select a configured role and use its **Fill priority** menu: **1st — Primary fill** reserves minimum slots when space allows; **2nd — Secondary fill** waits for a full percentage slot after primary allocations. For example, secondary Recon at 10% gets zero slots below 10 members in the assignment pool, one at 10, and two at 20 when space remains. This setting is separate from personal first/second-choice Discord roles and is saved in templates. Existing settings default to primary. Assignment winners are randomized within preference tiers; repeated winners can still occur by chance.
