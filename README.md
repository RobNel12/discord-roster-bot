...
Squad managers assigned to the summoned squad can click **🔓** to block self-service joins, moves into or out of the squad, and leaves. Members with Manage Server can lock or unlock any summoned squad. Explicit manager `/squad assign` and `/squad unassign` commands remain available. Locking does not block readiness reactions or alter voice-channel permissions. Locks survive restarts and are released by clicking **🔒** or automatically when the summons closes. Existing summons without a saved lock start unlocked.
