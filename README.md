...
Loadout configuration uses Discord's native role picker. Select `1st Medic` to configure Medic in the first fill group, or `2nd Recon` to configure Recon in the second fill group. Either variety can create a loadout directly and opens its percentage/instructions form. Selecting the other variety for an existing loadout switches its fill group while keeping its percentage, limits, instructions, and existing preference mappings. The most recently selected variety decides the group. Plain roles such as `Medic` default to the first group when configured. Percentages cannot total more than 100%; configuration pages paginate after 25 roles.

All first-group slots are considered before second-group slots. Within the first group, first-choice volunteers precede second-choice volunteers. The second group shuffles its roles each run and treats eligible first/second-choice volunteers equally; multiple optional `2nd [role]` preferences have no ranking among them. Both groups require a matching preference. Time served gives no advantage, and random draws can repeat winners. Existing saved fill groups are preserved; reselect a Discord role to change a group, then save a new template if desired. The separate fill-priority dropdown has been removed.
...
4. Select `2nd Recon` to create a second-group Recon loadout. You do not need to configure `1st Recon` first. Members' exact `2nd Medic` roles already qualify as fallback preferences for first-group Medic; selecting `2nd Medic` in configuration instead switches Medic to the second fill group.
5. Repeat for other specialist roles, then press **Done**. Any percentage not allocated to specialists becomes Rifleman.
6. Have eligible members join that squad's temporary voice channel, then press **Assign loadouts**. The bot refreshes their Discord roles, fills the first group, then the second group, and converts specialist vacancies without volunteers to Rifleman. Assignments are shown in the roster and manager response; assignment DMs remain paused.
...
The bot allocates first-group minimums and percentage targets, then second-group minimums and targets, all within maximum caps and squad size. First-group contention uses higher percentages, then configuration order; second-group contention uses its shuffled order. Default minimums remain 1 for the first group and 0 for the second group, so second-group Recon at 10% needs 10 members unless given an explicit minimum. A second-group minimum cannot displace a first-group target. Specialists require a matching preference; unmet minimums appear in the manager's summary. Remaining members become Rifleman; if Rifleman has a maximum, excess fallback members display **Unassigned loadout** while keeping squad membership.
...
Server managers can publish an automatically updated leaderboard with `/squad set-leaderboard-channel channel:#rank-leaderboard`. Officer and enlisted ranks appear on separate embed pages, each with its own numbering; large tracks continue onto additional pages. The bot edits existing messages without pinging and updates them with squad/member changes, rank thresholds, and periodic reconciliation. Deleted pages are recreated on refresh. It also works when no role or squad roster is published. Use the same command to move it; `/squad clear-leaderboard-channel` disables it and removes its pages, retrying failed deletions later. `/squad refresh` refreshes it manually. These dedicated commands manage the leaderboard rather than `/roster move` or `/roster delete`.
